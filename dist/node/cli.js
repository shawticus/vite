...
var build = require('./chunks/dep-19140984.js');
var os = require('os');
...
async function serve(config, port = 5000) {
    ...
    const server = await build.resolveHttpServer(config, app);
...
cli
    ...
    // is ok here
    const { createServer } = await Promise.resolve().then(function () { return require('./chunks/dep-19140984.js'); }).then(function (n) { return n.index$1; });
    try {
...
cli
    ...
    .option('-m, --mode <mode>', `[string] set env mode`)
    .action(async (root, options) => {
    const { build: build$1 } = await Promise.resolve().then(function () { return require('./chunks/dep-19140984.js'); }).then(function (n) { return n.build$1; });
...
cli
    ...
    .option('--force', `[boolean] force the optimizer to ignore the cache and re-bundle`)
    .action(async (root, options) => {
    const { optimizeDeps } = await Promise.resolve().then(function () { return require('./chunks/dep-19140984.js'); }).then(function (n) { return n.index; });
    try {
